...
class MusicXMLParser:
    ...
    def _expand_trem(self, i):
        step = SUBDIV_MAP['32nd'] # Simplified
        rem = i['nominal_q']
        on = i['onset_q']
        while rem > 1e-4:
            d = min(step, rem)
            self._add_raw(i['xml'], i['midi'], on, d, i['staff'], i['velocity'], i['finger_gt'], i['tempo'])
            on += d; rem -= d

    def _expand_trem_pair(self, a, b):
        step = SUBDIV_MAP['32nd']
        tot = a['nominal_q'] + b['nominal_q']
        on = a['onset_q']
        cnt = int(tot/step)
        for k in range(cnt):
            src = a if k%2==0 else b
            self._add_raw(a['xml'], src['midi'], on, step, a['staff'], a['velocity'], a['finger_gt'], a['tempo'])
            on += step
    ...
    def _expand_orn(self, i):
        """
        Implémentation complète de l'expansion des ornements (Trilles, Mordants, etc.)
        Adaptée pour écrire directement dans _raw_notes.
        """
        # Mapping XML -> Motif interne
        orn_map = {
        ...
        if not motif_key:
            ...
            self._add_raw(i['xml'], i['midi'], i['onset_q'], i['nominal_q'], i['staff'], i['velocity'], i['finger_gt'], i['tempo'])
            return

        prefix, body, suffix = ORNAMENT_MOTIFS[motif_key]
        bps = max(i['tempo'], 1.0) / 60.0

        # Choix de la subdivision temporelle selon le tempo (logique originale)
        if motif_key == 'Trill':
            if bps < 1.8: t_sub = SUBDIV_MAP['32nd'] / 10.0 # Très lent -> trille très rapide
            elif bps < 3.0: t_sub = SUBDIV_MAP['32nd']
            else: t_sub = SUBDIV_MAP['16th']
        else:
            if bps < 1.8: t_sub = SUBDIV_MAP['32nd']
            elif bps < 3.0: t_sub = SUBDIV_MAP['16th']
            else: t_sub = SUBDIV_MAP['eighth']
        ...
        body_len = len(body) * t_sub if body else 0.0
        reps = int(remaining / body_len) if body_len > 0 else 0

        # Génération de la séquence de pitchs
        ...
        seq_events = []
        for off in prefix: seq_events.append((get_neighbor(off), t_sub))
        for _ in range(reps):
            for off in body: seq_events.append((get_neighbor(off), t_sub))
        for off in suffix: seq_events.append((get_neighbor(off), t_sub))

        onset = i['onset_q']
        used_total = 0.0

        # Ajout des notes de l'ornement
        for midi, dur in seq_events:
            if dur > 0:
                self._add_raw(i['xml'], midi, onset, dur, i['staff'], i['velocity'], 0, i['tempo']) # GT finger 0 for ornaments
                onset += dur
                used_total += dur

        # Ajout de la note principale restante (sustain)
        remaining_main = max(MIN_NOTE_DURATION, nominal - used_total)
        self._add_raw(i['xml'], i['midi'], onset, remaining_main, i['staff'], i['velocity'], i['finger_gt'], i['tempo'])
...
def get_valid_mask(score: ScoreData) -> np.ndarray:
    """
