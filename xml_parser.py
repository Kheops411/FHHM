...
Dependencies: lxml
Usage: python musicxml_pianist_improved.py input.xml output.xml
"""
...
class PlayedNote:
    ...
    velocity: int
    tempo: float = DEFAULT_TEMPO  # BPM at note's position
    ...
    def onset_seconds(self) -> float:
        """Convert onset from quarters to seconds"""
        return quarters_to_seconds(self.onset, self.tempo)
    
    @property
    def duration_seconds(self) -> float:
        """Convert duration from quarters to seconds"""
        return quarters_to_seconds(self.duration, self.tempo)
    
    @property
    def offset_seconds(self) -> float:
        """Convert offset from quarters to seconds"""
        return quarters_to_seconds(self.offset, self.tempo)
...
def quarters_to_seconds(quarters: float, tempo_bpm: float) -> float:
    """Convert musical time (quarters) to absolute time (seconds)"""
    if tempo_bpm <= 0:
        tempo_bpm = DEFAULT_TEMPO
    return quarters * (60.0 / tempo_bpm)

def seconds_to_quarters(seconds: float, tempo_bpm: float) -> float:
    """Convert absolute time (seconds) to musical time (quarters)"""
    if tempo_bpm <= 0:
        tempo_bpm = DEFAULT_TEMPO
...
class MusicXMLParser:
    ...
    def _process_note(
        ...
        xml_fingering = self._get_xml_fingering(note)  # Extract ground truth
        ...
        note_info = {
            ...
            'xml_fingering': xml_fingering,  # Ground truth fingering
        }
    ...
    def _get_xml_fingering(self, note: etree._Element) -> Optional[int]:
        """Extract fingering from XML technical notations (ground truth)"""
        notations = note.find('notations')
        if notations is not None:
            technical = notations.find('technical')
            if technical is not None:
                fingering = technical.find('fingering')
                if fingering is not None and fingering.text:
                    try:
                        # Handle simple cases: "1", "2", etc.
                        # Handle substitutions: "1-2" → take last digit
                        # Handle alternations: "5" or "4" → take first
                        val_str = fingering.text.strip()
                        
                        # Remove common non-numeric characters
                        val_str = val_str.replace('(', '').replace(')', '').strip()
                        
                        # If contains hyphen (substitution), take last
                        if '-' in val_str:
                            val_str = val_str.split('-')[-1].strip()
                        
                        # If contains spaces, take first
                        if ' ' in val_str:
    ...
    def _create_played_note(
        ...
        **kwargs
        ...
        """Factory method to create PlayedNote with consistent parameters"""
        # On s'assure que la durée n'est jamais nulle pour éviter des divisions par zéro plus tard
        safe_duration = max(MIN_NOTE_DURATION, duration)
        
        return PlayedNote(
            hand=Hand.LEFT if note_info['staff'] == 2 else Hand.RIGHT,
            pitch=note_info.get('midi'),
            onset=onset,
            duration=safe_duration,
            offset=onset + safe_duration,
            velocity=note_info.get('velocity', DEFAULT_VELOCITY),
            tempo=note_info.get('tempo', DEFAULT_TEMPO),
            xml_element=note_info.get('xml'),
            source_tag=source_tag,
            voice=note_info.get('voice', '1'),
            staff=note_info.get('staff', 1),
            measure_number=note_info.get('measure_number', 0),
            finger=note_info.get('xml_fingering'), # Ground Truth
            extra=kwargs.get('extra', {})
        )

    def _append_simple_note(self, note_info: dict):
        """Append a simple note to played_notes"""
        # Calcul de la durée réelle basée sur l'articulation (staccato, etc.)
        duration = note_info['nominal_q'] * note_info['art_ratio']
        
        # Utilisation de la Factory pour garantir la cohérence (Tempo, Finger, etc.)
        played_note = self._create_played_note(
            note_info, 
            note_info['onset_q'], 
            duration, 
            'note'
    ...
    def _flush_graces_for_principal(
        ...
        for grace in graces:
            if grace['grace_type'] == 'Acciaccatura':
                ...
                t = ((2.0 / 3.0) if is_dotted else 0.5) * principal_duration / max(1, n)
            durations.append(t)
        ...
        for grace, duration in zip(graces, durations):
            ...
            # Create grace note info for factory
            grace_info = {
                'staff': grace['staff'],
                'midi': midi,
                'velocity': grace['velocity'],
                'tempo': DEFAULT_TEMPO,  # Graces typically follow context tempo
                'xml': grace['xml'],
                'voice': grace['voice'],
                'measure_number': grace['measure_number'],
                'xml_fingering': None,  # Graces typically don't have fingerings
            }
    ...
    def _expand_arpeggio_group(self, members: List[dict], state: PartState):
        ...
        # Detect direction
        is_descending = any(
            m['xml'].find('.//arpeggiate') is not None and
            'down' in m['xml'].find('.//arpeggiate').get('direction', '').lower()
            for m in members
        )
    ...
    def _expand_chord_group(self, members: List[dict], state: PartState, arpeggio: bool = True):
        """Expand a regular chord group (non-arpeggiated)"""
        if not members:
            return
    ...
    def _expand_tremolo(self, note_info: dict):
        ...
        t_step = {1: SUBDIV_MAP['eighth'], 2: SUBDIV_MAP['16th']}.get(beams, SUBDIV_MAP['32nd'])
        ...
        while remaining > 1e-9:
            ...
            played_note = self._create_played_note(note_info, onset, duration, 'tremolo')
            self.played_notes.append(played_note)
            onset += duration
            remaining -= duration
    ...
    def _expand_tremolo_two_notes(self, note_A_info: dict, note_B_info: dict):
        ...
        for i in range(steps_count):
            source_note = note_A_info if i % 2 == 0 else note_B_info
            played_note = self._create_played_note(
                {**note_A_info, 'midi': source_note['midi']},
                onset,
                t_step,
                'tremolo_two_notes'
            )
    ...
    def _expand_glissando(self, note_start_info: dict, note_end_info: dict):
        ...
        pitches = (list(range(pitch_start, pitch_end + 1)) if pitch_start <= pitch_end
                   else list(range(pitch_start, pitch_end - 1, -1)))
        
        if not pitches:
            return
        
        total_duration = note_start_info['nominal_q']
        t_step = max(GLISSANDO_MIN_NOTE_DURATION, total_duration / len(pitches))
        onset = note_start_info['onset_q']
        
        for pitch in pitches:
            played_note = self._create_played_note(
                {**note_start_info, 'midi': pitch},
                onset,
                t_step,
                'glissando'
            )
    ...
    def _expand_ornament(self, note_info: dict):
        ...
        motif_key = next((ornament_map.get(orn) for orn in note_info['ornaments']
                         if ornament_map.get(orn) in ORNAMENT_MOTIFS), None)
        ...
        if motif_key == 'Trill':
            if bps < 1.8: # Lent
                t_sub = SUBDIV_MAP['32nd'] / 10.0 # Trille très rapide
            elif bps < 3.0: # Moyen
        ...
        def midi_for_offset(offset: int) -> Optional[int]:
            return get_diatonic_neighbor(
                note_info['step'], note_info['octave'], offset,
                note_info['accmap_snapshot'], note_info['keysig_map']
            )
        
        # Build and generate sequence
        sequence = [(midi_for_offset(off), t_sub) for off in prefix]
        sequence.extend([(midi_for_offset(off), t_sub) for _ in range(reps) for off in body])
        sequence.extend([(midi_for_offset(off), t_sub) for off in suffix])
        ...
        for midi, duration in sequence:
            if duration > 0:
                ornament_info = {**note_info, 'midi': midi, 'xml_fingering': None}
                
                played_note = self._create_played_note(
                    ornament_info, onset, duration, f'ornament:{motif_key}',
        ...
        # Append shortened principal
        remaining_main = max(MIN_NOTE_DURATION, nominal - used_total)
        principal = self._create_played_note(
            note_info, note_info['onset_q'] + used_total,
            remaining_main, 'note_after_ornament'
        )
